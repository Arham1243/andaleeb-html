...
@section('content')
    <div class="col-md-12">
        <div class="dashboard-content">
            {{ Breadcrumbs::render('user.travel-insurances.pay-again', $insurance) }}
            <div class="custom-sec custom-sec--form">
                <div class="custom-sec__header">
                    <div class="section-content">
                        <h3 class="heading">Pay for Insurance: {{ $insurance->insurance_number ?? '' }}</h3>
                    </div>
                </div>
            </div>
            <form action="{{ route('user.travel-insurances.pay-again.proceed', $insurance->id) }}" method="POST" enctype="multipart/form-data"
                id="validation-form">
                @csrf
                @method('POST')
                <div class="row">
